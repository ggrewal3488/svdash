...
Both `ch375.js` and `acr120.js` are confirmed working against the real RD-08E and RW-41 hardware — `bridge32/verify.js` passes cleanly end to end. The dump/diff derivation ran on-site on 2026-08-24: room number, checkin, and checkout are fully located (see `docs/VENDOR_SDK_FINDINGS.md`), but a per-card checksum (block 1 byte 4, block 2 byte 15) was confirmed — by direct hardware test, not guesswork — to be a keyed value that can't be derived from card contents alone. `device-service/src/cardLayout.ts#encodeGuestCard()` stays a deliberate throw as a result; `POST /reservations/:id/cards` in `server/` already treats that as expected and tells the front desk to hand-encode via `btlock57.exe`, which is the real, permanent answer here short of reverse-engineering the vendor binary's compiled logic. `device-service/` itself still only proxies health/version checks and a `/cards/encode` stub through to `bridge32/`.
