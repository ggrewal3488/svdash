...
Both `ch375.js` and `acr120.js` are now confirmed working against the real RD-08E and RW-41 hardware (2026-08-22) — `bridge32/verify.js` passes cleanly end to end, and `dumpCard.js` successfully authenticates a real card and reads real block data. What's blocking real card encoding now isn't the DLL bindings anymore — it's that Godrej rekeyed every sector on cards it issues, so the sector keys need to be captured (see the "BLOCKED ON" note at the top of `src/cardLayout.ts` and `docs/VENDOR_SDK_FINDINGS.md`) before the dump/diff derivation plan can find the actual room-card byte layout. `device-service/` itself still only proxies health/version checks and a `/cards/encode` stub through to `bridge32/` — see `src/cardLayout.ts` for the full plan.
